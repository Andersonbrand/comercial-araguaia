        ...
        <header>
            <div className="header-container">
                <NavLink to="/" className="logo" onClick={() => setMenuOpen(false)}>
                    <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" viewBox="0 0 542 550">
                ...
                <div className={`header-nav-wrapper${menuOpen ? ' header-nav-wrapper--open' : ''}`}>
                    <nav>
                        <NavLink className="links-nav" to="/" end onClick={() => setMenuOpen(false)}>Home</NavLink>
                        <NavLink className="links-nav" to="/produtos" onClick={() => setMenuOpen(false)}>Produtos</NavLink>
                        <NavLink className="links-nav" to="/carrinho" onClick={() => setMenuOpen(false)}>Carrinho</NavLink>
                        <NavLink className="links-nav" to="/orcamento" onClick={() => setMenuOpen(false)}>Orçamento</NavLink>
                        {isAdmin && (
                            <NavLink className="links-nav links-nav--admin" to="/admin" onClick={() => setMenuOpen(false)}>Admin</NavLink>
                        )}
                    </nav>
                    ...
                    <div className="header-user">
                        {user?.name && (
                            <div className="header-user__profile">
                                <svg className="header-user__avatar" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none">
                                    <circle cx="12" cy="12" r="11" fill="rgba(255,255,255,0.12)" stroke="rgba(255,255,255,0.35)" strokeWidth="1.2" />
                                    <circle cx="12" cy="9" r="3.2" fill="rgba(255,255,255,0.9)" />
                                    <path d="M5.5 19.5c0-3.59 2.91-6.5 6.5-6.5s6.5 2.91 6.5 6.5" stroke="rgba(255,255,255,0.9)" strokeWidth="1.4" strokeLinecap="round" fill="none" />
                                </svg>
                                <span className="header-username">Olá,  {user.name}</span>
                            </div>
                        )}
